...
(function($, window, document) {
	...
	$.fn.dataTable = function( oInit )
	{
		...
		{
         //Jeads - The test for null needs to be the 
         // first condition otherwise the call to sData.replace
         // generates an error
			if ( sData === null )
			{
				return '';
			}
			else if ( typeof _oExt.ofnSearch[sType] == "function" )
			{
		...
		function _fnGetObjectDataFn( mSource )
		{
			...
			else if ( typeof mSource == 'string' && mSource.indexOf('.') != -1 )
			{
				...
				{
					return function (data) {
                  // If a column has all undefined values the
			         // data[ a[0] ][ a[1] ] generated a data[ a[0] ]
                  // undefined error.  Set undefined array elements to
                  // a string.  Jeads
                  if(data[ a[0] ] === undefined){
                     return '';
                  }else{
			            return data[ a[0] ][ a[1] ];
                  }
