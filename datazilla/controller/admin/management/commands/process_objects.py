...
Take a look at model.base.PerformanceTestModel.
...
datazilla/model/sql/objectstore.json.
    ...
    PerformanceTestModel methods that access 'objectstore' would access the appropriate data hub
    ...
    You can also read some of the methods in PerformanceTestModel to get an idea of
    what you can do with execute().

Note: Now that there are two different data hubs the PerformanceTestModel.dhub class
...
and call them through PerformanceTestModel methods that you add.


The manage.py command will utilize PerformanceTestModel methods that you add to retrieve the
objects and you can just call the existing method, PerformanceTestModel.load_test_data() to
...
manage.py command that transfers the data using PerformanceTestModel method calls.
