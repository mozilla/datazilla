...
class TestSetTestData(object):
    ...
    def test_happy(self, client, ptm):
        """Can set test data in objectstore via API endpoint."""
        response = client.oauth_post(ptm, {"test": "foo"})
    ...
    def test_oauth_error(self, client, ptm):
        ...
        with self._patch("oauth.Server.verify_request") as mock_verify:
            ...
            response = client.oauth_post(ptm, {}, status=403)
    ...
    def test_no_data(self, client, ptm):
        response = client.oauth_post(ptm, None, status=400)

        assert response.json["status"] == u"No POST data found"


    def test_malformed_json(self, client, ptm):
        response = client.oauth_post(ptm, "{[[[", status=400)
    ...
    def test_unknown_error(self, client, ptm):
        def _raise(*args, **kwargs):
            raise Exception("boom!")

        with self._patch("PerformanceTestModel.store_test_data") as mock_store:
            mock_store.side_effect = _raise
            response = client.oauth_post(ptm, {}, status=500)

        assert response.json["status"] == "Unknown error"
        assert response.json["message"] == "boom!"


    def test_bad_project(self, client, ptm):
        ptm.project = "doesnotexist"
        response = client.oauth_post(ptm, {}, status=404)
